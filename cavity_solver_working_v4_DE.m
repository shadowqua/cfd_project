function [PrsMatrix, uvelMatrix, vvelMatrix] = cavity_solver_working_v3_DE(~)
...
cb = contourcbar;
%cb.Layout.Tile = 'east';
title("Velocity in the u direction")
xlabel("Node of x position")
ylabel("Node of y position")
...
title("Velocity in the v direction")
xlabel("Node of x position")
ylabel("Node of y position")
...
cb = contourcbar;
%cb.Layout.Tile = 'east';
cb.Title.String = "Pressure";

title("Pressure")
xlabel("Node of x position")
ylabel("Node of y position")

%visdiff(filename1,filename2) %jump to here
...
u(1,1,1)=((two*u(1,2,1)-u(1,3,1))+(two*u(2,1,1)-u(3,1,1)))/2;

u(imax,jmax,1)=((two*u(imax,jmax-1,1)-u(imax,jmax-2,1))+(two*u(imax-1,jmax,1)-u(imax-2,jmax,1)))/2;

u(1,jmax,1)=((two*u(1,jmax-1,1)-u(1,jmax-2,1))+(two*u(2,jmax,1)-u(3,jmax,1)))/2;

u(imax,1,1)=((two*u(imax,2,1)-u(imax,3,1))+(two*u(imax-1,1,1)-u(imax-2,1,1)))/2;
...
%global dtvisc uvel2 beta2 lambda_x lambda_y lambda_max dtconv
...
nu=rmu/rho;
dtvisc=dx*dy/four/nu;
uvel2=zeros(imax,jmax);
vvel2=zeros(imax,jmax);

for j=2:jmax-1
    for i=2:imax-1
        uvel2(i,j)=u(i,j,2).*u(i,j,2);
        vvel2(i,j)=u(i,j,3).*u(i,j,3);

        beta2(i,j)=max(uvel2(i,j)+vvel2(i,j),rkappa*vel2ref);
        lambda_x(i,j)= half*(abs(u(i,j,2))+sqrt(u(i,j,2)^2+four*beta2(i,j)));
        lambda_y(i,j)= half*(abs(u(i,j,3))+sqrt(u(i,j,3)^2+four*beta2(i,j)));
        lambda_max(i,j)=max(lambda_x(i,j),lambda_y(i,j));

        dtconv(i,j)=min(dx,dy)/lambda_max(i,j);
        dt(i,j)=cfl*min(dtconv(i,j),dtvisc);
...
%global uvel2 beta2 d4pdx4 d4pdy4 %lambda_x lambda_y 
%cx is c4 for x, y for cy
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        if j==2
            lowerNodeX=two*u(i,j-1,1)-u(i,j,1);
            upperNodeX=u(i,j+2,1);
        elseif j==jmax-1
            upperNodeX=two*u(i,j+1,1)-u(i,j,1);
            lowerNodeX=u(i,j-2,1);
        else
            lowerNodeX=u(i,j-2,1);
            upperNodeX=u(i,j+2,1);
        end
        ...
        if i==2
            lowerNodeY=two*u(i-1,j,1)-u(i,j,1);
            upperNodeY=u(i+2,j,1);
            
        elseif i==imax-1
            upperNodeY=two*u(i+1,j,1)-u(i,j,1);
            lowerNodeY=u(i-2,j,1);
        else
            lowerNodeY=u(i-2,j,1);
            upperNodeY=u(i+2,j,1);
        end
        ...
        d4pdx4=(upperNodeX-four*u(i,j+1,1)+six*u(i,j,1)-four*u(i,j-1,1)+lowerNodeX)/dx^4;
        d4pdy4=(upperNodeY-four*u(i+1,j,1)+six*u(i,j,1)-four*u(i-1,j,1)+lowerNodeY)/dy^4;
...
% for j=3:jmax-2
%     artviscx(2,j)=two*artviscx(3,j)-artviscx(4,j);
%     artviscx(imax-1,j)=two*artviscx(imax-2,j)-artviscx(imax-3,j);
% 
%     artviscy(2,j)=two*artviscy(3,j)-artviscy(4,j);
%     artviscy(imax-1,j)=two*artviscy(imax-2,j)-artviscy(imax-3,j);
% 
% end
% 
% for i=3:imax-2
%     artviscy(i,2)=two*artviscy(i,3)-artviscy(i,4);
%     artviscy(i,jmax-1)=two*artviscy(i,jmax-2)-artviscy(i,jmax-3);
% 
%     artviscx(i,2)=two*artviscx(i,3)-artviscx(i,4);
%     artviscx(i,jmax-1)=two*artviscx(i,jmax-2)-artviscx(i,jmax-3);
% 
% end
% 
% 
...
% 
% 
% artviscx(2,2)=two*artviscx(3,2)-artviscx(4,2);
% artviscx(imax-1,jmax-1)=two*artviscx(imax-2,jmax-1)-artviscx(imax-3,jmax-1);
% 
% artviscy(2,2)=two*artviscy(2,3)-artviscy(2,4);
% artviscy(imax-1,jmax-1)=two*artviscy(imax-1,jmax-2)-artviscy(imax-1,jmax-3);
% 
% artviscx(2,jmax-1)=two*artviscx(3,jmax-1)-artviscx(4,jmax-1);
% artviscx(imax-1,2)=two*artviscx(imax-2,2)-artviscx(imax-3,2);
% 
% artviscy(2,jmax-1)=two*artviscy(2,jmax-2)-artviscy(2,jmax-3);
% artviscy(imax-1,2)=two*artviscy(imax-1,3)-artviscy(imax-1,4);
...
%global dpdx dudx dvdx dpdy dudy dvdy d2udx2 d2vdx2 d2udy2 d2vdy2 beta2 uvel2

%uold is needed since when you change something you still need to be able
%to reference the point it used to be when looking at other points (for
%example if you change point 3,3 then point 4,3 cares about it still and
%needs the old reference value, so change u while using uold)
...
% dpdx=zeros(imax,jmax);
% dudx=zeros(imax,jmax);
% dvdx=zeros(imax,jmax);
% dpdy=zeros(imax,jmax);
% dudy=zeros(imax,jmax);
% dvdy=zeros(imax,jmax);
% 
% d2udx2=zeros(imax,jmax);
% d2udy2=zeros(imax,jmax);
% d2vdx2=zeros(imax,jmax);
% d2vdy2=zeros(imax,jmax);
...
for j=2:jmax-1
    for i=2:imax-1
        % dpdx=(uold(i,j+1,1)-uold(i,j-1,1))/(two*dx);
        % dudx=(uold(i,j+1,2)-uold(i,j-1,2))/(two*dx);
        % dvdx=(uold(i,j+1,3)-uold(i,j-1,3))/(two*dx);
        % 
        % dpdy=(uold(i+1,j,1)-uold(i-1,j,1))/(two*dy);
        % dudy=(uold(i+1,j,2)-uold(i-1,j,2))/(two*dy);
        % dvdy=(uold(i+1,j,3)-uold(i-1,j,3))/(two*dy);
        % 
        % d2udx2=(uold(i,j+1,2)+uold(i,j-1,2)-two*uold(i,j,2))/(dx^2);
        % d2udy2=(uold(i+1,j,2)+uold(i-1,j,2)-two*uold(i,j,2))/(dy^2);
        % 
        % d2vdx2=(uold(i,j+1,3)+uold(i,j-1,3)-two*uold(i,j,3))/(dx^2);
        % d2vdy2=(uold(i+1,j,3)+uold(i-1,j,3)-two*uold(i,j,3))/(dy^2);
        dpdx=(u(i,j+1,1)-u(i,j-1,1))/(two*dx);
        dudx=(u(i,j+1,2)-u(i,j-1,2))/(two*dx);
        dvdx=(u(i,j+1,3)-u(i,j-1,3))/(two*dx);

        dpdy=(u(i+1,j,1)-u(i-1,j,1))/(two*dy);
        dudy=(u(i+1,j,2)-u(i-1,j,2))/(two*dy);
        dvdy=(u(i+1,j,3)-u(i-1,j,3))/(two*dy);

        d2udx2=(u(i,j+1,2)+u(i,j-1,2)-two*u(i,j,2))/(dx^2);
        d2udy2=(u(i+1,j,2)+u(i-1,j,2)-two*u(i,j,2))/(dy^2);

        d2vdx2=(u(i,j+1,3)+u(i,j-1,3)-two*u(i,j,3))/(dx^2);
        d2vdy2=(u(i+1,j,3)+u(i-1,j,3)-two*u(i,j,3))/(dy^2);
        
        % pfij=artviscx+artviscy;
        % ufij=rmu*(d2udx2+d2udy2);
        % vfij=rmu*(d2vdx2+d2vdy2);
        % 
        % 
        % u(i,j,1)=half*dy^2/(dx^2+dy^2)*(uold(i-1,j,1)+uold(i+1))+half*(dx^2/(dx^2+dy^2))*(uold(i,j-1,1)+uold(i,j+1,1))-half*(dx^2*dy^2/(dx^2+dy^2))*pfij;
        % u(i,j,2)=half*dy^2/(dx^2+dy^2)*(uold(i-1,j,2)+uold(i+1))+half*(dx^2/(dx^2+dy^2))*(uold(i,j-1,2)+uold(i,j+1,1))-half*(dx^2*dy^2/(dx^2+dy^2))*ufij;
        % u(i,j,3)=half*dy^2/(dx^2+dy^2)*(uold(i-1,j,3)+uold(i+1))+half*(dx^2/(dx^2+dy^2))*(uold(i,j-1,3)+uold(i,j+1,1))-half*(dx^2*dy^2/(dx^2+dy^2))*vfij;
        ...
        %dt or dtmin?
        u(i,j,1)=uold(i,j,1)-beta2(i,j)*dt(i,j)*(rho*dudx+rho*dvdy-S(i,j)-fmass);
        u(i,j,2)=uold(i,j,2)-dt(i,j)/rho*(rho*u(i,j,2)*dudx+rho*u(i,j,3)*dudy+dpdx-rmu*d2udx2-rmu*d2udy2-fxmtm);
        u(i,j,3)=uold(i,j,3)-dt(i,j)/rho*(rho*u(i,j,2)*dvdx+rho*u(i,j,3)*dvdy+dpdy-rmu*d2vdx2-rmu*d2vdy2-fymtm);
...
if n==1
    ...
    % resinit(2) = sqrt(sum(abs((u(:,:,2)-uold(:,:,2))./dt(:,:)).^2,'all')/(imax*jmax));%L2 norm of continuity
    
    resinit(1)=1;
    resinit(2)=1;
    resinit(3)=1;
    
    %resinit(3) =%sqrt(sum(abs((u(:,:,3)-uold(:,:,3))./dt(:,:)).^2,'all')/(imax*jmax));%L2 norm of continuity
    %was 0 so rip
...
% L2old=[sqrt(sum(abs(resinit(:,:,1)))/(imax*jmax)),sqrt(sum(abs(resinit(:,:,2)))/(imax*jmax)),sqrt(sum(abs(resinit(:,:,3)))/(imax*jmax))];
% L2new=[sqrt(sum(abs(res(:,:,1)))/(imax*jmax)),sqrt(sum(abs(res(:,:,2)))/(imax*jmax)),sqrt(sum(abs(res(:,:,3)))/(imax*jmax))];
...
%L1(1)=sqrt(sum(abs(u(:,:,1)-s(:,:,1)),'all')/(imax*jmax))

% grid41=readmatrix('cavity41 - Copy.dat');
% grid65=readmatrix('cavity65 - Copy.dat');
% grid81=readmatrix('cavity81 - Copy.dat');
% grid33=readmatrix('cavity33 - Copy.dat');
% 
% pDE41=grid41(:,9);
% uDE41=grid41(:,10);
% vDE41=grid41(:,11);
...
%L1p=[sum(abs(pDE33))/(33*33),sum(abs(pDE41))/(41*41),sum(abs(pDE65))/(65*65),sum(abs(pDE81))/(81*81)];

%L2p=[sqrt(sum(abs(pDE33).^2)/(33*33)),sqrt(sum(abs(pDE41).^2)/(41*41)),sqrt(sum(abs(pDE65).^2)/(65*65)),sqrt(sum(abs(pDE81).^2)/(81*81))];

%Linfp=[max(abs(pDE33)),max(abs(pDE41)),max(abs(pDE65)),max(abs(pDE81))];

%L1u=[sum(abs(uDE33))/(33*33),sum(abs(uDE41))/(41*41),sum(abs(uDE65))/(65*65),sum(abs(uDE81))/(81*81)];

%L2u=[sqrt(sum(abs(uDE33).^2)/(33*33)),sqrt(sum(abs(uDE41).^2)/(41*41)),sqrt(sum(abs(uDE65).^2)/(65*65)),sqrt(sum(abs(uDE81).^2)/(81*81))];

%Linfu=[max(abs(uDE33)),max(abs(uDE41)),max(abs(uDE65)),max(abs(uDE81))];

%L1v=[sum(abs(vDE33))/(33*33),sum(abs(vDE41))/(41*41),sum(abs(vDE65))/(65*65),sum(abs(vDE81))/(81*81)];

%L2v=[sqrt(sum(abs(vDE33).^2)/(33*33)),sqrt(sum(abs(vDE41).^2)/(41*41)),sqrt(sum(abs(vDE65).^2)/(65*65)),sqrt(sum(abs(vDE81).^2)/(81*81))];

%Linfv=[max(abs(vDE33)),max(abs(vDE41)),max(abs(vDE65)),max(abs(vDE81))];

%rL1norm=[L1p',L1u',L1v'];
%rL2norm=[L2p',L2u',L2v'];
%rLinfnorm=[Linfp',Linfu',Linfv'];
...
pDE = DE(:,:,1);
uDE=u(:,:,2)-ummsArray(:,:,2);
vDE=u(:,:,3)-ummsArray(:,:,3); % same thing tbh

L1 = [sum(abs(pDE),"all")/(imax*jmax), sum(abs(uDE),"all")/(imax*jmax), sum(abs(vDE),"all")/(imax*jmax)];
L2 = [sqrt(sum(abs(pDE).^2, "all")/(imax*imax)), sqrt(sum(abs(uDE).^2, "all")/(imax*imax)), sqrt(sum(abs(vDE).^2, "all")/(imax*imax))];
Linf = [max(abs(pDE),[],"all"), max(abs(uDE),[],"all"), max(abs(vDE),[],"all")];

rL1norm = L1'
rL2norm = L2'
rLinfnorm = Linf'

deltaX = xmax/imax % use for h calculation

% Input these values into an excel for manual plotting.
