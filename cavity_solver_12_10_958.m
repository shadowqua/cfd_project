function [PrsMatrix, uvelMatrix, vvelMatrix] = cavity_solver_post9(~)
...
imax = 161;   	% Number of points in the x-direction (use odd numbers only)
jmax = 161;   	% Number of points in the y-direction (use odd numbers only)
...
u(1,1,1)=mean(two*u(1,2,1)-u(1,3,1),(two*u(2,1,1)-u(3,1,1)));
u(imax,jmax,1)=mean(two*u(imax,jmax-1,1)-u(imax,jmax-2,1),(two*u(imax-1,jmax,1)-u(imax-2,jmax,1)));
u(1,jmax,1)=mean(two*u(1,jmax-1,1)-u(1,jmax-2,1),(two*u(2,jmax,1)-u(3,jmax,1)));
u(imax,1,1)=mean(two*u(imax,2,1)-u(imax,3,1),(two*u(imax-1,1,1)-u(imax-2,1,1)));
...
global uvel2 beta2 lambda_x lambda_y d4pdx4 d4pdy4 
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        d2udx2=(uold(i+1,j,2)+uold(i-1,j,2)-two*uold(i,j,2))/(dx^2);
        d2udy2=(uold(i,j+1,2)+uold(i,j-1,2)-two*uold(i,j,2))/(dy^2);

        d2vdx2=(uold(i+1,j,3)+uold(i-1,j,3)-two*uold(i,j,3))/(dx^2);
        d2vdy2=(uold(i,j+1,3)+uold(i,j-1,3)-two*uold(i,j,3))/(dy^2);
...
L2old=[sqrt(mean(resinit(:,:,1))),sqrt(mean(resinit(:,:,2))),sqrt(mean(resinit(:,:,3)))];
L2new=[sqrt(mean(res(:,:,1))),sqrt(mean(res(:,:,2))),sqrt(mean(res(:,:,3)))];



if n~=ninit
    conv=max(L2new./L2old);
else
    conv=99.9;
end
