function [PrsMatrix, uvelMatrix, vvelMatrix] = cavity_solver_post11(~)
...
for j=1:jmax
    for i=1:imax
        ...
        lambda_x= max(lambda_x,half*(abs(u(i,j,2))+sqrt(u(i,j,2)^2+four*beta2(i,j))));
        lambda_y= max(lambda_y,half*(abs(u(i,j,3))+sqrt(u(i,j,3)^2+four*beta2(i,j))));
    end
end
...
for j=3:jmax-2
    for i=3:imax-2
        ...
        artviscx(i,j)=-lambda_x*Cx*dx^3/beta2(i,j)*d4pdx4;
        artviscy(i,j)=-lambda_y*Cy*dy^3/beta2(i,j)*d4pdy4;
    end
end
...
for j=1:jmax
    for i=1:imax
        beta2(i,j)=max(u(i,j,2)+u(i,j,3),rkappa*vel2ref);
    end
end
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        d2udx2=(u(i+1,j,2)+u(i-1,j,2)-two*u(i,j,2))/(dx^2);
        d2udy2=(u(i,j+1,2)+u(i,j-1,2)-two*u(i,j,2))/(dy^2);

        d2vdx2=(u(i+1,j,3)+u(i-1,j,3)-two*u(i,j,3))/(dx^2);
        d2vdy2=(u(i,j+1,3)+u(i,j-1,3)-two*u(i,j,3))/(dy^2);
